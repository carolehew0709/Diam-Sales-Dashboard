...
type PortalLine = { product: string; customer: string; customerType: 'External' | 'Group'; dgc: 'E' | 'G'; total2026: string; total2027: string; monthly2026: string[]; monthly2027: string[] };
const emptyLine = (): PortalLine => ({ product: '', customer: '', customerType: 'External', dgc: 'E', total2026: '', total2027: '', monthly2026: months.map(() => ''), monthly2027: months.map(() => '') });
...
export function ImportPortal({ currentUser }: { currentUser: User }) {
  ...
  const updateMonth = (lineIndex: number, monthIndex: number, value: string, year: 'monthly2026' | 'monthly2027') => setLines((items) => items.map((line, index) => index === lineIndex ? { ...line, [year]: line[year].map((month, itemIndex) => itemIndex === monthIndex ? value : month) } : line));
  const submit = async () => {
  ...
  return <section className="import-page" id="import-center">
    ...
    <div className="panel portal-panel"><div className="panel-heading"><div><span className="kicker">Step 03 · Monthly phasing</span><h2>2026 orderbook allocation</h2></div><span className="unit">Optional detail</span></div><div className="table-wrap portal-phase"><table><thead><tr><th>Order line</th>{months.map((month) => <th key={month}>{month}</th>)}</tr></thead><tbody>{lines.map((line, lineIndex) => <tr key={lineIndex}><td><strong>{line.product || `Line ${lineIndex + 1}`}</strong><small>{line.customer || 'Customer pending'}</small></td>{months.map((month, monthIndex) => <td key={month}><input aria-label={`${month} 2026 allocation`} inputMode="decimal" value={line.monthly2026[monthIndex]} onChange={(event) => updateMonth(lineIndex, monthIndex, event.target.value, 'monthly2026')} placeholder="-"/></td>)}</tr>)}</tbody></table></div><div className="portal-phase-title">2027 orderbook allocation</div><div className="table-wrap portal-phase"><table><thead><tr><th>Order line</th>{months.map((month) => <th key={month}>{month}</th>)}</tr></thead><tbody>{lines.map((line, lineIndex) => <tr key={lineIndex}><td><strong>{line.product || `Line ${lineIndex + 1}`}</strong><small>{line.customer || 'Customer pending'}</small></td>{months.map((month, monthIndex) => <td key={month}><input aria-label={`${month} 2027 allocation`} inputMode="decimal" value={line.monthly2027[monthIndex]} onChange={(event) => updateMonth(lineIndex, monthIndex, event.target.value, 'monthly2027')} placeholder="-"/></td>)}</tr>)}</tbody></table></div></div>
